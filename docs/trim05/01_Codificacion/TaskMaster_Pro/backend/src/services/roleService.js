...
class RoleService {
    ...
    async updateRole(id, data) {
        try {
            const getRole = await db.query(`SELECT * FROM roles WHERE id = ?`, [id]);
            ...
            if (getRole.length != 0) {
                var dataQry = [data.nombre, data.descripcion];
                const results = await db.query(`UPDATE roles SET nombre=?, descripcion=? WHERE id=?`, [...dataQry, id]);
                if (results.length != 0) {
    ...
    async deleteRole(id) {
        try {
            const getRole = await db.query(`SELECT * FROM roles WHERE id  = ?`, [id]);
            if (getRole.length != 0) {
                const results = await db.query(`DELETE FROM roles WHERE id = ?`, [id]);
                if (results.length != 0) {
